...
final <- data.frame(id = test_set$id, actual = test_set$romantic, predicted = prediction)
errorRate <- final[final$actual != final$predicted,] %>% nrow / final %>% nrow

library(ggplot2)
ggplot(final) +
  geom_point(aes(x = id, y = actual), color = "blue") + 
  geom_point(aes(x = id, y = predicted), color = "red")
